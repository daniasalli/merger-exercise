...
<div class="detail-container">
  ...
  <div class="p-3" v-for="key in infoKeys" :key="key">
    ...
    <div v-if="hasProperties(key) && detailProperties.length">
    ...
    <tr v-for="detail in detailProperties" :key="detail">
...
export default {
  ...
  computed: {
    infoKeys() {
      return Object.keys(this.data).filter(key => key !== 'sum');
    },
  },
  methods: {
    hasProperties(key) {
      this.detailProperties = Object.keys(this.data[key]);
      return this.detailProperties.length > 0;
    },
    closeAlert() {
